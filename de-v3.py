...
# ====== New Relic Setup (Top of File) ======
import os

# Write newrelic.ini using license key from environment or secrets
newrelic_ini_content = f"""
common:
  license_key = {os.environ.get("NEW_RELIC_LICENSE_KEY", "MISSING")}
  app_name = BitLocks-Decryption
  log_level = info
  audit_mode = false
  enabled = true
...
# Hardcoded AES key (or use secrets in production)
key = b'\xbf\x1b\xb3O\x8fB\x88e\x04\xea\xfb\xcd{.\xa9\xdc<\xef\xeb\xb9\x08\x10\xd3\x18\x92\x0f\xb6\x80\xe1 <V'
iv = b'P\x05\x95\xac\xf5\x88\x9c\x1a\x89\x94 ^\x92i\xc8\xbc'
...
# Inputs
key2 = st.text_input("Enter Key (not used here)")
ipfs_link = st.text_input("Enter IPFS link")
...
if st.button("Fetch File from IPFS"):
    if ipfs_link:
        ...
        if file_content:
            ...
            st.warning("Failed to retrieve file from IPFS. Please check the IPFS link.")
    else:
        st.warning("Please enter the IPFS link.")

# Decrypt uploaded image
output_file_path = 'Cell6.txt'
uploaded_file = st.file_uploader("Drag and drop an image here", type=["jpg", "jpeg", "png"])
if uploaded_file:
    decrypt_image(uploaded_file, key, iv, output_file_path)
...
# Extract binary string from decrypted file
with open("Cell6.txt", "rb") as input_file, open("process.txt", "w") as output_file:
    input_bytes = input_file.read()
    input_str = input_bytes.decode("utf-8", errors="ignore")
    filtered_str = ''.join(c for c in input_str if c in {'0', '1'})
...
# Extract original file from binary
binary_file_path = "process.txt"
with open(binary_file_path, "r") as f:
    binary_string = f.read()
...
# Save original file
output_filename = "example3." + original_file_ext
